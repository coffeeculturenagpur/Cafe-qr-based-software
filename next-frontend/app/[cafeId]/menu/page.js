      ...
      {tableGuard.status === "checking" ? (
        ...
        <main className="min-h-screen">
          <div className="menu-topbar sticky top-0 z-20 border-b backdrop-blur-xl">
            <div className="mx-auto flex w-full max-w-md min-w-0 items-center justify-between gap-2 px-2 py-2.5 sm:px-4 sm:py-3">
              <div className="min-w-0 flex-1 px-1 sm:px-2">
                ...
                <div className="menu-muted mt-2 flex min-w-0 items-center gap-2 text-xs">
                  <MapPin size={12} strokeWidth={2.2} className="menu-text" />
                  <span className="min-w-0 truncate">{cafe?.address || "Freshly brewed, made to order."}</span>
                </div>
              </div>
          ...
          <div className="mx-auto w-full max-w-md min-w-0 px-1 pt-2 sm:px-4 sm:pt-3">
            <div className="menu-panel overflow-hidden rounded-3xl">
              <div
                className="relative h-36 w-full bg-slate-100 sm:h-40"
                style={{
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-lg font-semibold text-white">{cafe?.name || "Cafe"}</div>
                      <div className="mt-1 flex min-w-0 items-center gap-2 text-xs text-white/80">
                        <Sparkles size={12} />
                        <span className="truncate">Chef-curated picks for today</span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="-mx-1 mt-4 flex snap-x gap-2 overflow-x-auto px-1 pb-2 no-scrollbar">
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-10 shrink-0 snap-start whitespace-nowrap rounded-full px-4 py-2 text-xs font-semibold transition ${active
                  ...
                  <div className="bg-venue-gradient-soft border-venue-accent rounded-3xl border p-4">
                    ...
                    <div className="mt-3 grid gap-3">
                      {specials.map((it) => {
                        const inCart = cart.find((x) => x._id === it._id);
                        return (
                          <div key={it._id} className="menu-panel border-venue-accent flex min-w-0 items-center justify-between gap-3 rounded-2xl p-3">
                            <div className="min-w-0">
                              <div className="menu-text text-sm font-semibold">{it.name}</div>
                              <div className="menu-muted break-words text-xs">{it.description || "Chef recommended today."}</div>
                              <div className="text-venue-primary mt-1 text-xs font-semibold">INR {Number(it.price || 0).toFixed(0)}</div>
                            </div>
                            {!inCart ? (
                              <Button onClick={() => add(it)} className="h-8 shrink-0 rounded-full px-4 text-xs">
                            ...
                            <CardContent className="!p-0">
                              <div className="relative h-36 w-full sm:h-44">
                                {it.image ? (
                                  ...
                                  <div className="menu-card-image-fallback h-36 w-full sm:h-44" />
                                )}
                                {!available && (
                              ...
                              <div className="p-3 sm:p-4">
                                <div className="flex min-w-0 items-start justify-between gap-2">
                                  <div className="min-w-0">
                                    ...
                                    <div className="menu-muted mt-1 break-words text-xs">
                                      {it.description || "Fresh, handmade, and served warm."}
                                    </div>
                                  </div>
                                  <div className="menu-text shrink-0 text-sm font-bold">INR {Number(it.price || 0).toFixed(0)}</div>
                                </div>

                                <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                                  <div className="menu-muted flex min-w-0 flex-wrap items-center gap-2 text-xs">
